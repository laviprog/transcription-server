...
from src.utils.fs import remove_file
from src.workers import log
from src.workers.db import complete_task_sync, update_task_sync
...
class DBReportingTask(Task):
    ...
    def on_success(self, retval, task_id, args, kwargs):
        ...
        try:
            # Result and status are written together: the task is never reported as
            # COMPLETED unless its result is durably stored.
            complete_task_sync(
                UUID(task_id),
    ...
    def after_return(self, status, retval, task_id, args, kwargs, einfo):
        # Celery does not call after_return for RETRY/REJECTED, but guard anyway:
        # deleting the audio here would leave the next attempt with nothing to read.
